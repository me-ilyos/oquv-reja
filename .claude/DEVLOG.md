...
**Task:** Phase 2 of REFACTOR_INSTRUCTIONS.MD — behavior-preserving structural refactor (output byte-identical to the Phase-1 baseline for all three samples)
**Solution:** (2.1+2.2) Replaced the three numeric-coercion spellings with one `_to_int(value)->int|None` plus a guarded `_cell_int(row,col,default)`; the parser now stores numbers as numbers (`hours`/`credits` are `int|None`, semester maps `dict[int,int]`, breakdown fields default to 0 since a blank breakdown cell means zero hours), and all string formatting moved into `formatter.py` (`_cell` renders None as `""`, `_row` assembles rows). (2.3) Split the ~90-line `detect_course_columns` into `_find_anchor`, `_find_code_name_cols`, `_find_hours_col`, `_find_classroom_col`, `_find_subcategory_cols`; `_find_anchor` also replaced the duplicate '1.00' scan in `_detect_col_range`. (2.4) Unified the ~80%-identical parse loops behind one `_iter_section_rows(ws, num_col, section)` generator (single home for the Jami/next-.00 stop) and factored the thrice-duplicated breakdown read into `_breakdown(row, cols, defaults)`. (2.5) Added `models.py` (`ColumnLayout` NamedTuple + `Course`/`Alternative`/`SelectiveSlot` dataclasses); `detect_columns(ws)` reads program duration once and returns (layout, semester_map, weekly_map) that `process_file` passes into both parse functions, cutting repeat scans (duration 4x->1x, anchor ~6x->3x); credit reconciliation moved to a `derived_credits` property. (2.6) Moved full direction resolution (incl. the read-cell-below variant) into `parser.resolve_direction`, so `main.py` is pure orchestration and no longer imports `re`. Finally condensed six restating helper docstrings. `detect_course_columns` (90->~20 lines) and both parse functions are substantially shorter; the data model moved to `models.py`. Note: `parser.py` total length is roughly flat (~487) because the required 2.3 decomposition adds per-helper overhead and `resolve_direction` moved in from `main.py`.
**Date:** 2026-06-09

---

**Task:** Phase 1 of REFACTOR_INSTRUCTIONS.MD — four correctness/robustness fixes to the parser (commit each separately)
**Solution:** (1.1) Fixed the real bug: the section-end check in `parse_core_courses` and `parse_selective_courses` tested `== "Jami:"` with a colon, but `60110100_ppd.xlsx`'s total row reads `Jami` (no colon), so selective slot 2.04 ran past the end and parsed 8 alternatives instead of 3 (swallowing `Jami`/`Malakaviy amaliyot`/`Yakuniy davlat attestatsiyasi`/`Jami`/`HAMMASI`). Normalized both checks to `str(cell).strip().rstrip(":").casefold() == "jami"`. (1.2) Added `_warn_credit_mismatch(num, hours_raw, semester_credits)` — after deriving `credits = hours/30`, compares against the sum of the per-semester credit values and prints a `WARNING:` on disagreement (flag only, no auto-correct); wired into both parse functions. All 66 core courses across the three samples reconcile with zero mismatches. (1.3) `_extract_program_years` now prints `WARNING: program duration not parsed; defaulting to 4 years` before the fallback, so a misdetected duration (which mis-maps every semester column) is observable; samples parse 4/4/3 so it stays silent. (1.4) Added `_clean_name` — collapses internal whitespace and neutralizes literal `|` at every name read site, since ministry templates use hard line breaks that would corrupt Markdown rows; collapsed one real double-space in `60110100_ppd` course 1.13. Net output change for the three samples: slot 2.04 shrinks 8→3 alternatives and the 1.13 double-space collapses; everything else byte-identical.
**Date:** 2026-06-09
